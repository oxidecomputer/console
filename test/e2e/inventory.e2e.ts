...
test('Disk inventory page', async ({ page }) => {
  ...
  await expectRowVisible(table, { id: physicalDisks[3].id, policy: 'in service' })
  await expectRowVisible(table, { id: physicalDisks[3].id, state: 'active' })
  await expectRowVisible(table, { id: physicalDisks[4].id, policy: 'expunged' })
  await expectRowVisible(table, { id: physicalDisks[4].id, state: 'active' })
  await expectRowVisible(table, { id: physicalDisks[5].id, policy: 'expunged' })
  await expectRowVisible(table, { id: physicalDisks[5].id, state: 'decommissioned' })
})
