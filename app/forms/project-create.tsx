...
import { api, apiq, queryClient, useApiMutation, type ProjectCreate } from '@oxide/api'
...
export default function ProjectCreateSideModalForm() {
  ...
  const createProject = useApiMutation(api.methods.projectCreate, {
    onSuccess(project) {
      // refetch list of projects in sidebar
      queryClient.invalidateEndpoint('projectList')
      // avoid the project fetch when the project page loads since we have the data
      const projectView = apiq(api.methods.projectView, { path: { project: project.name } })
