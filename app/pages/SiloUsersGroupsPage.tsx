...
// Groups doesn't trigger a fetch. This loader runs once on entry; react-router
// won't re-run it when navigating between sibling tab routes. Both tabs fetch
// the full user/group lists so they can be sorted by name client-side (the API
// only sorts by id).
...
export const handle = makeCrumb('Users & Groups', pb.siloUsers())

export default function SiloUsersGroupsPage() {
  return (
    <>
      <PageHeader>
        <PageTitle icon={<PersonGroup24Icon />}>Users &amp; Groups</PageTitle>
        <DocsPopover
          heading="users and groups"
          icon={<PersonGroup16Icon />}
          summary="Users and groups come from the silo's identity provider. Assign silo and project roles to control what they can view, edit, or administer."
          links={[docLinks.keyConceptsIam, docLinks.access, docLinks.identityProviders]}
        />
      </PageHeader>
      <RouteTabs fullWidth>
        <Tab to={pb.siloUsers()}>Users</Tab>
        <Tab to={pb.siloGroups()}>Groups</Tab>
      </RouteTabs>
    </>
  )
}
