...
const staticCols = [
  ...
  colHelper.accessor('subnetPoolId', {
    header: 'Subnet pool',
    cell: (info) => <SubnetPoolCell subnetPoolId={info.getValue()} />,
  }),
  colHelper.accessor('instanceId', {
    header: 'Instance',
    cell: (info) => <InstanceLink cell instanceId={info.getValue()} tab="networking" />,
  }),
]
...
export default function ExternalSubnetsPage() {
  ...
  useQuickActions(
    () => [
      {
        value: 'New external subnet',
        navGroup: 'Actions',
        action: pb.externalSubnetsNew({ project }),
      },
      ...(allSubnets?.items || []).map((s) => ({
        value: s.name,
        action: pb.externalSubnetEdit({ project, externalSubnet: s.name }),
        navGroup: 'Edit external subnet',
      })),
    ],
    [project, allSubnets]
  )
  ...
  return (
    ...
    >
      <Message
        ...
        content={
          <>
            The selected instance will be reachable at subnet <HL>{subnet.subnet}</HL>
          </>
        }
      />
      <ListboxField
