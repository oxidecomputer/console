...
import { api, apiq, getListQFn, queryClient, useApiMutation, type Image } from '@oxide/api'
...
const imageList = getListQFn(api.methods.imageList, {})
...
const PromoteImageModal = ({ onDismiss }: { onDismiss: () => void }) => {
  ...
  const projects = useQuery(apiq(api.methods.projectList, {}))
...
const DemoteImageModal = ({
  ...
  const demoteImage = useApiMutation(api.methods.imageDemote, {
    onSuccess(data) {
      addToast({
