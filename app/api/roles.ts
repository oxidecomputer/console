...
import { api, apiq, usePrefetchedQuery } from './client'
...
export function useUserRows(
  ...
  const { data: users } = usePrefetchedQuery(apiq(api.methods.userList, {}))
  const { data: groups } = usePrefetchedQuery(apiq(api.methods.groupList, {}))
  return useMemo(() => {
