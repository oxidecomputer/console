...
export function Bounds({
  ...
  const onFinishRef = React.useRef<(() => void) | undefined>(onFinish)
  ...
  useFrame((state, delta) => {
    ...
    if (animationState.current === AnimationState.START) {
      ...
      if (t.current >= 1) {
        ...
        goal.current.camPos &&
          camera.position.lerpVectors(origin.current.camPos, goal.current.camPos, k)
        goal.current.camRot &&
          camera.quaternion.slerpQuaternions(origin.current.camRot, goal.current.camRot, k)
