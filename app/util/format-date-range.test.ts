 ...
 *
 * Copyright Vercel
 * Copied from: https://github.com/vercel/little-date
 */
import { describe, expect, test } from 'vitest'

import { formatDateRange, type DateRangeFormatOptions } from './format-date-range'
...
describe('format date range', () => {
  ...
  test('format date range different year', () => {
    ...
    expect(formatDateRange(from, to, defaultOptions)).toBe("Jan 1 '22 - Jan 20 '23")
  })
  ...
  test('format date range same day, different hours', () => {
    ...
    expect(formatDateRange(from, to, defaultOptions)).toBe('Jan 1, 12:11am - 2:30pm')
  })
  ...
  test('format date range different days with time', () => {
    ...
    expect(formatDateRange(from, to, defaultOptions)).toBe('Jan 1, 12:11am - Jan 2, 2:30pm')
  })
  ...
  test('format date range different days with single time', () => {
    ...
    expect(formatDateRange(from, to, defaultOptions)).toBe('Jan 1, 12:11am - Jan 2')
  })
  ...
  test('format date range hour difference', () => {
    ...
    expect(formatDateRange(from, to, defaultOptions)).toBe('Jan 1, 12pm - 12:59pm')
  })
