...
export const handlers = makeHandlers({
  ...
  instanceCreate({ body, query, cookies }) {
    const project = lookup.project(query)
    ...
    if (body.enable_jumbo_frames && !jumboFramesOptIn(cookies)) {
      throw invalidRequest(
  ...
  instanceUpdate({ path, query, body, cookies }) {
    ...
    if (body.enable_jumbo_frames === true && !jumboFramesOptIn(cookies)) {
      throw invalidRequest(
  ...
  systemNetworkingSettingsView: ({ cookies }) => {
    requireFleetViewer(cookies)
    return { external_jumbo_frames_opt_in_enabled: jumboFramesOptIn(cookies) }
  },
